...
public class GameController implements Initializable{
    ...
    private MyNode[][] simulation;
    private Organism[][] organism;
    private Organism[][] theGrid;
    private int gridWidth;
    private int gridHeight;


   private static class MyNode extends StackPane {

        private MyNode( Organism organism, double x, double y, double width, double height) {

            Rectangle rectangle = new Rectangle( width, height);
            rectangle.setStroke(Color.(organism.getColor()));
            rectangle.setFill(Color.(organism.getColor()));
            // set position
            setTranslateX( x);
            setTranslateY( y);
    ...
    private void infoOfCell(MouseEvent event){
        if (paused) {
            ...
            String name = source.getColor();                                                   //getColor() returns color of offspring
            for (int i = 0; i < 30; i++) {
                for (int j = 0; j < 30; j++) {
                    if (organism[i][j].getColor().name) {
    ...
    @FXML
    private void viewListOfEnviro( ObservableList<String> listOfEnvironment){
    ...
    @FXML
    private void updateSurvivalRate( double rateOfSurvival ) {
